...
def get_z_score_plot(symbol, currency):
    ...
    data['Z-Score'] = (data['Close'] - data['Close'].rolling(window=511).mean()) / data['Close'].rolling(window=511).std()
    
    # Plotting
    fig = go.Figure()
    
    # Bitcoin closing prices (log scale)
    fig.add_trace(go.Scatter(x=data.index, y=data['Close'], mode='lines', name=symbol + ' Price', line=dict(color='orange')))
    
    # Z-score
    fig.add_trace(go.Scatter(x=data.index, y=data['Z-Score'], mode='lines', name='Z-Score', line=dict(color='deeppink'), yaxis='y2'))
